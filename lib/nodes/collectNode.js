  ...
  instance: {
    ...
    __createMatches: function(lc) {
      ...
      me.setAliases.forEach(function(alias, i) {
        var lcValue = lcFh[alias],
          idx;
        //
        if (me.isCollectionObject) {
    ...
    _createMatch: function(lc, fc, rc) {
      ...
      while (++i < l) {
        if (!eqConstraints[i](fh, fh)) {
          // fh: all the 'regular' bindings plus a single collection binding
          createdContext = DEFAULT_MATCH; // this is a non-match; e.g. lc.isMatch() => false
          break;
        }
      }
      if (!createdContext) {
        lc.fromMatches[fc.hashCode] = createdContext = lc.clone(
          rc.fact,
          null,
          match
        );
    ...
    modifyLeft: function(lc, retract) {
      //   console.log("before", this.collectionHash);
      var me = this,
      ...
      if (ctx) {
        ...
        try {
          me.setAliases.forEach(function(alias) {
            ...
            if (me.isCollectionObject) {
              // a hash of sets -> { key: [values], key2: [values], ... }
              const theArray = collection[alias];
              const idx = extd.findIndex(theArray, lcOldValue);
              retract ? theArray.splice(idx, 1) : (theArray[idx] = lcNewValue);
              if (retract && empty) {
                empty = theArray.length ? false : true;
              }
            } else {
              // an array either of single values OR [{hash of single values}, {...},...]
              if (1 === me.setAliases.length) {
                // a simple array
                const idx = extd.findIndex(collection, lcOldValue);
                retract
                  ? collection.splice(idx, 1)
                  : (collection[idx] = lcNewValue);
              } else {
                // we have a hash remove the entire thing from the array
                collection.some(function(x, i) {
                  if (x.filteredHashCode === fc.hashCode) {
                    idx = i;
                    return true;
                  }
                });
                ...
                if (!retract) {
                  if (!setHash) {
                    setHash = {};
                    setHash.filteredHashCode = fc.hashCode;
                  ...
                  if (empty) {
                    empty = collection.length ? false : true;
                  }
                }
              }
            }
      ...
      //   console.log("after", this.collectionHash);
    },
    //
    ...
    getFilteredContext: function(lc) {
      ...
      extdObj(match.factHash).forEach(function(val, key) {
        clone.set(key, val);
      });
      return clone;
    },
    ...
    getFilteredHashCode: function(context) {
      ...
      context.match.aliases.forEach(function(alias, i) {
        if (
          extd.findIndex(me.setAliases, function(x) {
            return x == alias;
          }) === -1
        ) {
