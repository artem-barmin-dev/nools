var extd = require("../extended"),
  keys = extd.hash.keys,
  Node = require("./node"),
  LeftMemory = require("./misc/leftMemory"),
  RightMemory = require("./misc/rightMemory");
...
Node.extend({
  instance: {
    ...
    retractLeft: function(context) {
      const afterRetract = this.removeFromLeftMemory(context);
      if (afterRetract) {
        context = afterRetract.data;
    ...
    retractRight: function(context) {
      const afterRetract = this.removeFromRightMemory(context);
      if (afterRetract) {
        context = afterRetract.data;
    ...
    assertLeft: function(context) {
      ...
      var rm = this.rightTuples.getRightMemory(context),
        i = -1,
        l = rm.length;
      while (++i < l) {
    ...
    modifyLeft: function(context) {
      const afterModify = this.removeFromLeftMemory(context);
      if (afterModify) {
        var previousContext = afterModify.data;
        this.__addToLeftMemory(context);
        var rm = this.rightTuples.getRightMemory(context),
          l = rm.length,
          i = -1,
        ...
        if (!l) {
          ...
          while (++i < l) {
            this.propagateAssertModifyFromLeft(
              context,
              rightMatches,
              rm[i].data
            );
    ...
    modifyRight: function(context) {
      const afterModify = this.removeFromRightMemory(context);
      if (afterModify) {
        var previousContext = afterModify.data;
        this.__addToRightMemory(context);
        var lm = this.leftTuples.getLeftMemory(context);
        if (!lm.length) {
          ...
          var leftMatches = previousContext.leftMatches,
            i = -1,
            l = lm.length;
          while (++i < l) {
            this.propagateAssertModifyFromRight(
              context,
              leftMatches,
              lm[i].data
            );
    ...
    propagateFromLeft: function(context, rc) {
      this.__propagate(
        "assert",
        this.__addToMemoryMatches(
          rc,
          context,
          context.clone(null, null, context.match.merge(rc.match))
        )
      );
    },
    ...
    propagateFromRight: function(context, lc) {
      this.__propagate(
        ...
        this.__addToMemoryMatches(
          context,
          lc,
          lc.clone(null, null, lc.match.merge(context.match))
        )
      );
    },
      ...
      if (factId in rightMatches) {
        this.__propagate(
          ...
          this.__addToMemoryMatches(
            rm,
            context,
            context.clone(null, null, context.match.merge(rm.match))
          )
        );
      } else {
      ...
      if (factId in leftMatches) {
        this.__propagate(
          ...
          this.__addToMemoryMatches(
            context,
            lm,
            context.clone(null, null, lm.match.merge(context.match))
          )
        );
      } else {
    ...
    removeFromRightMemory: function(context) {
      ...
      if (context) {
        ...
        var hashCodes = keys(leftMatches),
          i = -1,
          l = hashCodes.length;
        while (++i < l) {
    ...
    removeFromLeftMemory: function(context) {
      ...
      if (context) {
        ...
        var hashCodes = keys(rightMatches),
          i = -1,
          l = hashCodes.length;
        while (++i < l) {
    ...
    getRightMemoryMatches: function(context) {
      var lm = this.leftMemory[context.hashCode],
        ret = {};
      if (lm) {
        ret = lm.rightMatches;
      }
