...
Customer.prototype = {
  ...
  getOrder: function(name) {
    var item;
    this.items.some(function(order) {
      if (order.type === name) {
        item = order;
        return true;
      }
...
it.describe("collect condition", function(it) {
  it.describe("basic test of collection element", function(it) {
        ...
        flow.rule(
          ...
          [
            ...
            [
              Array,
              "list",
              "list.size === c.items.size",
              "from collect( item : Item item.price > 10 from c.items )"
            ]
          ],
          function(facts) {
            debugger;
      ...
      function(flow) {
        ...
        flow.rule(
          "rule 2",
          {
            salience: 5,
            noLoop: true,
            scope: { Customer: Customer, Item: Item }
          },
    ...
    //
    it.should(
      "use set in from expression with modified fact causing rhs to fire once",
      function() {
        var session = flow.getSession();
        //
        var customer = new Customer("John");
  ...
  it.describe("basic test of collect using DSL", function(it) {
    var defines = { Customer: Customer, Item: Item },
      flow = nools.compile(__dirname + "/collect.nools", {
        name: "TestCollect",
        defines: defines
      }),
      rule1Called = 0;
